---
title: Raw Data
subtitle: Understanding the structure of the raw data
layout: page
show_sidebar: false
---
...
* **initial_run_flown**: These are the initial tests run for both the Anafi drone and the MIT Flightgoggles drone. It contains the runs using random selection, maximum velocity selection as well as kinematic and dynamic selection. No scoring function are used in any of these tests.
* **handcrafted_run_flown**: This contains the test data for handcrafted scoring models. These tests were only run on the MIT Flightgoggles drone.
* **learned_run_flown**: This contains the test data for learned scoring models. There is a different scoring model for each of the MIT Flightgoggles drone controllers.T
* **anafi_learned_run_flown**: This is the test data for the learned scoring model of the Anafi drone.

For example, you will see these folders if you run the `ls` command in the `FinalResults` directory:

![The ls command output](../img/first.png)

## Data Structure

Each of the data folders contains the data generated for the specific experiment. Below is a description of how the data is structured.

### Test Generation Output

The folder contains a list of text files and folders pairs. The text files contain the terminal output from running the test generation tool. The folders contain the tests generated for each run. Thus by counting the folders, we can see that there are a total of 43 test sets in the initial run folder. The folders and text file pairs were named with all generation parameter information in the name.

So, for instance, the text file:
```
...
* Search type (*kinematic*): The type of search model used. The model can be either random, maxvel, or kinematic.
* Score type (*random*): The type of scoring model used. The scoring model can either be random, learned, or a name given to the type of handcrafted scoring technique.
* Drone Type (*anafi*): The type of physical drone used. In our studies, we targeted two physical drones, namely the Anafi and MIT drone.
...
* Simulation time (*27*): The total time in seconds giving to the simulator or outdoor test runs. This time is used by the test generation technique to take into account how much time is required to execute an identified test.
...
For example, you will see these folders if you run the `ls` command and grep for any files or folders with length 3 in the `FinalResults/initial_run_flown` directory:
...
Inside each of the folders, you will find a 'maps' folder. The maps folder contains the data generated by a test. Each map is a new test which the robot should execute. If we were to run the `ls` command inside the map folder we would have the following output:
...
Each of the controllers then produced the following files.
...
There are a few files in here which are then computed by the data processing section described later. However, for completion, a brief description of each file is given below:
...
* Trajectory Length (10): AThe number of waypoints in a trajectory. This includes the start and end waypoint.
...
More in-depth details about how to process the data under the [Data Processing](./rq1/) tab. However, here is a basic example of using raw data to get something meaningful. Let's say we wanted to visualize the experiment run on the learned scoring model of the MIT drone using a waypoint controller. We would navigate to the following folder:
