...
We have broken the instructions into two sections. First we introduce how to get started with the test generation. Second we introduce how to get started with the test execution.  The test generation is relatively simple. However, running the simulator and executing the tests is complex, with many dependencies on both the graphics and the networks cards of the computer and are thus not suitable for a virtual machine. We however have still provided instructions on how to setup the simulator on a standalone machine. (**Note:** We ran our tests on an Ubuntu 16.04 machine with 64 gigs of ram, i9-10900X, and a Titan RTX with 24 gigs of onboard memory).


# Test Generation

This section describes the test generating process used in our study. In our study we set the total time of test generation to 2 hours. Due to the time constraints for evaluating the tool, we have instead used a total time of 1 minute. We will start by describing how to generate the tests followed by how to process the tests. To generate the tests go into the `TestGeneration\GenerateTests` folder. To do that run the command:

```bash
cd ~/Artifact/TestGeneration/GenerateTests
```

Once you are inside this folder you can use the python script to generate tests. The `main.py` script will start the test generation process. It however takes in the parameters to start the test generation process. Each of the parameters are described in the table below:

Name    | Description
------------ | -------------
drone               |  The type of drone used, this only is important if the maxvel or kinematic search type is used.
...
score               |  The score model used. It can be no scoring model (**random**), one of the predefined handcrafted models (**edge, edge90, edge180**), or a learned scoring model (**learned**)
trajectorylength    |  The length of the trajectory you wish to create, this includes the start and end point.
...
Using this knowledge lets generate tests, using the kinematic model of the MIT drone, with no scoring model and of length 5. Lets set the total time to 60 seconds and set and estimate that execution in simulation will take 5 seconds per test. To do that run the following command:
...
You will get the following output with the total number of valid trajectories dependant on the computational resources available to the virtual machine.
...
You will notice that the *Total overall time* is slightly longer than the time given. This is the time starting all the required classes as well as saving both the text and renders of each of the trajectories. The results will be saved inside the `Results` folder. To view the results you can run:

```bash
tree -d -L 4 --filelimit=10
```

You will see the each of the valid trajectories saved in a corresponding maps results folder as shown below:

![The ls command output](../img/testgeneration.png)

We can the repeat this experiment for both longer trajectories as well as different search types. For instance running if we run the same experiment as above except use no search type by setting it to random:
...
We notice that due to the simplification of the search algorithm we are able to process many more explored trajectories, however we find no valid trajectories. We would need to run the experiment much longer before a valid trajectory was found. The next step in the process would be executing the tests.
...
At this stage you should have a simulator similar to the one below. (**Note:** We were testing custom maps in the simulator in this run, and so we expect your drone would be much higher up without any walls. However you will see the drone and the waypoints)

![smaller](../img/minimap.gif)
